...
function PendingApprovalsPage() {
  ...
  const rows: PendingRow[] = [
    ...(agenciesQuery.data?.data ?? []).map((a: Agency): PendingRow => ({
      id: `agency-${a.id}`,
      kind: 'agency',
      typeLabel: 'PR Agency',
      icon: Building2,
    ...
    ...(outletsQuery.data?.data ?? []).map((o: Outlet): PendingRow => ({
      id: `outlet-${o.id}`,
      kind: 'outlet',
      typeLabel: 'Outlet',
      icon: Store,
    ...
    ...(requestsQuery.data?.data ?? []).map((r: AdminRequest): PendingRow => ({
      id: `request-${r.id}`,
      kind: 'request',
    ...
    ...(jobsQuery.data?.data ?? []).map((j: SpecialService): PendingRow => ({
      id: `job-${j.id}`,
      kind: 'job',
