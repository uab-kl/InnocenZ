/**
 * ── LOCALE BOUNDARY: this document is ENGLISH in every UI locale. ──
 *
 * Deliberate, not an unfinished translation. A payment voucher is a financial
 * record an agency files, sends to the payee and may hand to an auditor, so the
 * artefact must be a function of the VOUCHER — not of whichever operator's UI
 * language happened to click Export.
 *
 * Three reasons, heaviest first:
 *
 * 1. The PR's copy of this same voucher is generated by the backend
 *    (`features/payment-voucher/payment-voucher-pdf.ts` and `-excel.ts`), which
 *    is hardcoded English and takes no locale. Everything below is its
 *    cell-for-cell twin — that is the entire purpose of this file. Translating
 *    this half alone would put two different-looking documents behind one
 *    voucher number, which is precisely the fault this file was written to end.
 * 2. One voucher, two operators, two languages of PDF. A record whose shape
 *    changes with the reader's settings is not a record.
 * 3. The typography cannot carry it. The HTML sets Arial/Helvetica and the
 *    workbook hardcodes Arial 10pt with "Segoe Script" on the signature line —
 *    none of which has CJK glyphs — while the column widths (13/30/10/14/14)
 *    and the row-height heuristic `description.length / 32` are counted in
 *    Latin characters.
 *
 * So there is no `t` in this file. The export UI AROUND it — the buttons, the
 * letterhead-loading notice, the layout hint — lives in the caller components
 * and is translated there (`agencyPv.loadingLetterhead`,
 * `agencyPv.letterheadUnavailable`, `agencyPv.printLayoutNote`).
 *
 * If this is ever revisited: translate BOTH exporters together, ship a CJK font
 * with the workbook, and take the language from the voucher, never from the
 * session. A half-translated voucher is worse than either language.
 *
 * ⚠️ One string below is dead, and is NOT a candidate for anything. The
 * "Print → Save as PDF · <ref>" hint at the end of the body carries class
 * `no-print`, and the stylesheet declares `.no-print { display: none; }`
 * unconditionally — not inside an `@media print` block — so it renders nowhere:
 * not in the preview tab, not on paper. Reported as a follow-up rather than
