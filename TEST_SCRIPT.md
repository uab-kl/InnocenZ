...
> images were cut. Hard-refresh `/` to verify. **Still open:** convert multi‑MB landing PNGs to
> WebP/AVIF (P3 below) — this pass fixed interaction/compositing jank, not payload size.
...
- [ ] **🟡 Landing assets still multi‑MB PNGs** — §8 X58 fixed interaction/compositing lag on `/`, but `venue-lounge.png` / `venue-neon-floor.png` / agency benefit photos remain ~2MB each. Convert to WebP/AVIF at display size; keep lazy-load. *(Web / landing)*
...
> mobile kill switches. **Still open (§9 P3):** convert multi‑MB landing PNGs to WebP/AVIF.
> Files: `handoff/primitives.tsx`, `HandoffPlatform.tsx`, `HandoffChallenges.tsx`,
> `HandoffBenefits.tsx`, `landing-handoff.css`.
