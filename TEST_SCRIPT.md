...
| X15 | **The agency Payroll & PV list cannot show ANY real voucher** — Atlas has 4 (2 `pending_review`, 2 `sent`) and both week tabs render "Payment Vouchers (0)". `pvBelongsToPayrollWeek` requires `pv.weekStartIso === <tab week start>`, but tab weeks come from the DEMO timeline and start on **Sunday** (19 Jul, 12 Jul) while the backend's `week_start` is **Monday** (20 Jul, 27 Jul) — the strings can never be equal. Compounding it, neither tab covers the CURRENT week, which is where `pending_review` vouchers sit. The detail view is reachable only by deep link (`/en/agency/pv?pv=<uuid>`), because `detail` is looked up over the unfiltered list | **Agency ← PR** | `routes/agency/pv.tsx:152` | live | 🔴 **Confirmed defect — open** |
...
- [ ] **🔴 Make real vouchers reachable on the agency Payroll list** *(SL — needs a product call)*. §8 X15: the week tabs are derived from the DEMO timeline and start on **Sunday**, the backend's `week_start` is **Monday**, so `pvBelongsToPayrollWeek`'s string equality never matches and the list shows 0 of Atlas's 4 real vouchers; neither tab covers the current week either. Everything on `PvDetail` — the day review, the receipt evidence, the send button — is therefore only reachable by deep link `?pv=<uuid>`. **Not a one-line fix:** matching by week OVERLAP instead would surface the 20–26 Jul voucher but still hide current-week ones and could match two tabs at once, and the KPI header labels the tab's week, so listing real vouchers under a week label they do not belong to would be a fresh label bug of the class already catalogued 6 times here. Decide whether the Payroll screen keeps two demo-derived week tabs at all once real vouchers exist for arbitrary weeks.
- [ ] **⚠️ Nothing tells an agency a voucher is waiting for review** *(SL — needs a product call)*. The payout job holds it at `pending_review` and logs `awaiting agency day review`; no notification is raised. The panel makes the queue visible to somebody who goes looking — it does not make anyone look. A new notification kind is the owner's call; there are 6 today.
