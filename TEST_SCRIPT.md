...
- ☐ **RBAC** — role / permission / module / pending → **confirm role-gating blocks wrong roles**. Both gates now exist — retest with a real sign-in: (1) backend `requireAdmin` on admin-request routes returns 403 to non-admin tokens; (2) front door: opening ANY `/admin/*` URL while signed in as agency lands on `/agency`, outlet lands on `/outlet`, a PR token lands on `/no-access` — the admin shell must never render for them
