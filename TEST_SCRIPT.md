...
- [ ] **PV day review — the FRONTEND** *(SL)*. **Both decisions are now ANSWERED and the gates are shipped + live-verified (§8 X12).** **Service layer DONE** (`apps/web/src/services/payment-voucher/index.ts`): `PaymentVoucherDayReview` type, `dayReviews`/`allDaysReviewed`/`hasHeldDay` on `PaymentVoucherWithLines`, plus `reviewPaymentVoucherDay()` and `approveAllPaymentVoucherDays()`. Web tsc unchanged at the 121 baseline. **Remaining: the hook (`use-agency-pv-day-review`) and the panel on `routes/agency/pv.tsx`.** `GET /payment-voucher/:id` returns `dayReviews[]` + `allDaysReviewed` + `hasHeldDay`; write with `PATCH /:id/day-review/:date` (`{status:'approved'|'held'|null, note?}`) and `POST /:id/day-review/approve-all`. **The panel must read `hasHeldDay` for send-readiness, never `allDaysReviewed`** — both are true when one day is held. Gate the buttons on `agencyCan(role,'raisePv')` (owner + finance), matching the server's `agencyOwnerOrFinance`; the READ is deliberately ungated.
