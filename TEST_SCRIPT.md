...
- [ ] **PV day review — the FRONTEND** *(SL)*. Backend shipped `ef45445`: `GET /payment-voucher/:id` already returns `dayReviews[]` + `allDaysReviewed` + `hasHeldDay`, with `PATCH /:id/day-review/:date` and `POST /:id/day-review/approve-all`. What is left is the agency Payroll panel, and **deciding whether a held day BLOCKS issue** (recommended) plus **which agency sub-role may approve** — a money-authority call, not a technical one.
