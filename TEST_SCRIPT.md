...
- [ ] 🔥 **`/api/v1/user` has NO role guard** — **CONFIRMED LIVE 30 Jul:** a PR token (Vicky) called `GET /user?page=1&pageSize=1` and got **200 with `totalCount=17`** — every account on the platform, admins included. `PATCH /user/:id` is equally ungated. Guard: admin-only list, self-or-admin patch. *(security)*
- [ ] 🔥 **GraphQL `auditLogs` is not admin-guarded** — **CONFIRMED LIVE 30 Jul:** the same PR token read **865 audit rows** (the resolver's `@auth` directive only requires *any* login; the repository merely hides `role='admin'` rows). Hard-require admin in the resolver. *(security)*
- [ ] **Audit-log per-role page under-fills** — **CONFIRMED IN THE UI 30 Jul:** `/admin/audit-log/admin-audit` rendered **1 row** while the footer read *"Showing 1 – 10 of 2752 entries · Page 1 of 276"*, because `AuditLogFilterInput` has no `role` field and `filterLogsByRole` runs client-side **after** server pagination. Add `role` to the GraphQL filter so counts and rows agree. *(Admin ← all)*
...
- [ ] **"Reset demo" button sits in the header of every admin page** — it only calls `queryClient.invalidateQueries()` and toasts *"Demo data refreshed"* (no DB write), but the label reads like a destructive data wipe to a real admin. Rename to "Refresh" or hide outside demo builds. *(Admin · `components/layout/header.tsx:95`)*
